...
var NTTInfo = {
	...
	listId:[],
	listUname:[],
	renderNTTAvatar:function(){
		var k = document.getElementsByTagName('i');
		var time = new Date();
		time = time.getTime();
		for (var i=0; i<k.length; i++){
			var el =$(k[i]);
			if(el.hasClass('fn-NTT-info')){
				var elId = el.attr('id')?el.attr('id'):'NTTA'+time+'_'+i;
				var aSize = el.attr('NTT-avatar-size')?el.attr('NTT-avatar-size'):50;
				el.attr('id', elId);
				if(el.attr('NTT-uname')) this.listUname.push(el.attr('NTT-uname')+'?'+elId);
				else this.listId.push(el.attr('NTT-uid')+'?'+elId);
...
function fA(id,val,uname,dname){
	var el = $('#'+id);
	if(el.length){
		var img = $('#'+el.attr('NTT-avatar'));
		var link = $('#'+el.attr('NTT-link'));
		var eD = $('#'+el.attr('NTT-dname'));
		var NTT = 'http://me.zing.vn/';
		if(img.length){
			if(img[0].tagName=='IMG') img.attr('src',val);
		}
		if(link.length){
			if(link[0].tagName=='A') link.attr('href',NTT+uname);
		}
		if(eD.length){
			eD.html(dname)
			if(eD[0].tagName=='A') eD.attr('href',NTT+uname);
		}
		el.remove();
	}
			...
			$.getJSON(this.src+'&offset='+this.last, function(data, status, xhr){
				if(status=='success'){
					...
					$('.fn-formattime').each(function(){
						...
						if(match){
							val = val.replace(match[0], NTTInfo.formatDateTime(match[0]));
							$(this).html(val);
...
var MoSearch = {
			...
			if(k){
				...
				for(var i=0; i<kHistory.length; i++){
					if(kHistory[i].trim().toLowerCase().search(k) !=-1) {
						items+='<li><a href="/tim-kiem/bai-hat.html?q='+encodeURI(kHistory[i])+'">'+kHistory[i]+'</a></li>';
					}
				}
			} else {
				for(var i=0; i<kHistory.length; i++){
	...
	addHistory:function(k){
		...
		if(kHistory){
			...
			k = k.toLowerCase();
			for(var i=0; i<kHistory.length; i++){
				if(k==kHistory[i].trim().toLowerCase()) {
					has = true;
					break;
				}
			}
